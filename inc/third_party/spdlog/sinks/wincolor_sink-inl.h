...
{
	static auto s_string_convert = new std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>>();

	if (end > start) {
		auto size = end - start;
		std::wstring wstr;
		try {
			wstr = s_string_convert->from_bytes(formatted.data() + start, formatted.data() + end);
		} catch (...) {
			wstr = CA2W(std::string(formatted.data() + start, size).c_str());
		}
		auto ignored = ::WriteConsoleW(static_cast<HANDLE>(out_handle_), wstr.c_str(), wstr.length(), nullptr, nullptr);
		(void)(ignored);
	}
}
